...
#            pats.insert(ss.order, ss)
        ...
        for lzseq in lvlZeroSeq_Dict.viewkeys(): #viewvalues():  #To ADD 
...
#            print ("PARAMETERS EXCEPTION HERE")
        ...
        for k, v in patsMap.patDict.iteritems():
            print "values"
            print k, v
...
#                seq[p.id_parent].children.insert(p.order, item)
        ...
        for l in lista:
            elem = lvlzelems_dict[l.id_pae]
            item = Pathitem(l.id_pae ,elem.name, l.id_pathid, elem.paetype, l.id_parent, l.lvl, l.order, l.operator) #TO UPDATE ON 266
            item.gid = modsMap.putItem(idgen,elem,l.id_pathid,l.order,l.lvl)
            pats.insert(item.order,item)
...
#            pats.insert(ss.order, ss)
        ...
        for lzseq in lvlZeroSeq_Dict.viewkeys(): #viewvalues():  
            lzsequence = seq[lzseq] 
            pats.insert(lzsequence.order, lzsequence) 
...
#        
#        cK = queries.getDirectoryByName("/users/krajczar/HI2015/7_4_6_patch2",db, log)
#        configs = queries.getConfigsInDir(cK.id,db)
#        
#        print 'LEN configs', str(len(configs))
#        
#        for c in configs:
#            print c.name
#            if c.name == '/users/krajczar/HI2015/7_4_6_patch2/HIon':
#                versions = queries.getConfVersions(c.id,db, log)
#                print 'VERSIONS', str(len(versions))
        ...
        try:
            ...
            log.error(msg)
#            log.error('ERROR: Query getDirectoryByName Error')
            return None
...
#            print ("PARAMETERS EXCEPTION HERE")
        ...
        print "INSIDE EXPOSED VER " + str(ver) + " CNF " +  str(cnf) 
...
#        attrs = vars(description)
#        print ', '.join("%s: %s" % item for item in attrs.items())
#        attrs = vars(description)
        print "DESCRIPTION: ", description
#       
        if (description is not None):
            print "DESCRIPTION VALUE: ", description.description
    
#        for d in description:
#            print "DESC ITEMS: ", d.id, d.value #, d.id_val, d.id_var, d.name
#            attrs = vars(d)
#            print ', '.join("%s: %s" % item for item in attrs.items())
...
#                pd = PathDetails(path.id, path.name, labels, values, "", "")
                pd = None
                if (description is not None):
                    print "DESCRIPTION VALUE: ", description.description #description.value
                    pd = PathDetails(path.id, path.name, labels, values, "", description.description)

                else: 
                    pd = PathDetails(path.id, path.name, labels, values, "", "")
            else:
                print "DEFAULT 1 VALUES"
                labels_len = len (labels)
                print "LABLES_LEN: ", labels_len
                vals = [1] * labels_len 
...
#            print "VER CNF ERROR"
        ...
        elif(ver != -2 and ver != -1): #TO ADD IN APP 266
...
#            print ("PARAMETERS EXCEPTION HERE")
        ...
        resp = Response() #TO ADD
        schema = ResponseSummaryItemSchema() #TO ADD
        
        resp.success = False #TO ADD
        ...
        try:
            ...
            output = schema.dump(resp) #TO ADD
            return output.data #TO ADD
...
#        print "Got dprels_dict"
        ...
        if smartPre is None:
            log.error('ERROR: Smart prescale None')
        
        #------ Building default 1-values row ----------
        
        if labels is None: #TO ADD
            labels_len = 0
        else: 
            labels_len = len(labels)
        ...
        for d in datasets:
            if (d.id == -1):
                ...
                #                                print "ERROR NOT SAME CARDINALITY"
                                    ...
                                    #SMArt PReSCALE COLUMN!!!!!!
                                    sp_value = smart_paths[p.name]
                                    smpr = "smart_pre" + "###" + str(sp_value) 
                                    pat.values.append(smpr)
                                    
                                    #END SMArt PReSCALE COLUMN!!!!!!
                                    
                                    for c in columns:
            #                            sv = SummaryValue(c.name,values[i2])
                #                        print "VALUESSS: ", c.name, " - ", values[i2]
                #                        setattr(sv, c.name, values[i2])

                                        pre_value = int(values[i2])
                        
                                        new_value = sp_value*pre_value
                            
#                                        print 'PRE_VAL: ',pre_value,' * SMART: ',sp_value,' = ',new_value
...
#                print "got paths for ", d.name
                
#                si.children.extend(paths)
                si.gid = sumMap.put(idsumgen,si,"dat")
                
                
#                print "got gid and stream id "
            
                streams_dict.get(streamid).children.append(si)
                
#                print "put dataset ", d.name, " in stream"
...
#        print "got ordered stream dict values "
