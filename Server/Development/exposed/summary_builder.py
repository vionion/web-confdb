...
class SummaryBuilder():
    def getPrescaleColumns(self, version=None, queries=None, db=None, log = None):
        ...
        try:
            ...
            log.error('ERROR: Query getPrescaleTemplate Error')
            return None    
        ...
        try:    
            ...
            log.error('ERROR: Query getPrescale Error')
            return None    
        ...
        try:
            ...
            log.error('ERROR: Query getPrescalelabels Error')
            return None
...
#                   
    ...
    def getSmartPrescales(self, ver_id = -1, id_rel = -1, streams = None, queries=None, db=None, log = None):
        if (db == None or queries == None or streams == None or ver_id == -1 or id_rel == -1):
        ...
        try:
            ...
            msg = 'ERROR: Query getSmartPrescaleExpressions ' + e.args[0]
            log.error(msg)
            print str(triggerConditions)
            return None
        
#        print str(triggerConditions)
        for mi in triggerConditions:
            print str(mi.id_pae) + " - " + str(mi.name)
        
        triggerConditions_dict =  {}
        
        for x in triggerConditions:
            triggerConditions_dict[x.id_pae] = x
        
        print "--KEYS----"
        print triggerConditions_dict.keys()
        print "----------"
        for t in triggResFilters:
            print str(t.id_pae) + " - " + t.name
            
#        dict((x.id_pae, x) for x in triggerConditions) #TO DO - iS WRONG
    ...
    #                    print "SP EXPRESSION: " +trf_expression +"/n"
    ...
    #                            pre_str = pre_str.strip( '"' )
    #                            print 'AFTER TRANS pre_str: ', pre_str
    ...
    #                            print "TERM: ",ter,"PRE: ",prescale 
                                ...
                                print "SMART PRESCALE: " + str(ter) + " " + str(prescale)
                            
                        smart_prescales_dict[stream] = sp   
        
        print "RETURNING FROM SUMMARY BUILDER"
