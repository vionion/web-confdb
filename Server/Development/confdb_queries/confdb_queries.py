...
#        
#        Base.prepare(db.get_bind(),reflect=True)
...
#        elements = db.query(Pathelement).from_statement(text("SELECT UNIQUE u_paelements.id, u_paelements.name "
#						+ "FROM u_paelements, u_conf2pae " # u_pathid2pae,u_pathid2conf #, u_mod2templ
#						+ "WHERE u_conf2pae.id_confver=:ver " #u_pathid2conf.id_pathid=u_pathid2pae.id_pathid
#						+ "and u_conf2pae.id_pae=u_paelements.id "
#						+ "and u_paelements.paetype=1 "
##						+ "and u_mod2templ.id_pae=u_paelements.id "
#				        + "order by u_paelements.name ")).params(ver = id_ver).all() #++ and u_pathid2conf.id_confver=:ver
...
#            + "order by u_outmelements.id "
#        )).params(streamid=oumId).all()
...
#    def getL1SeedsModules(self, idis = None ,db=None, log = None):
#
#        if (db == None or idis == None):
##                print ("PARAMETERS EXCEPTION HERE")
#                log.error('ERROR: getDatasetPathids - input parameters error')
#
#        l1s = db.query(ModToTemp).filter(ModToTemp.id_templ == ModTemplate.id).filter(ModTemplate.name == 'HLTLevel1GTSeed').filter(ModToTemp.id_pae == Pathitems.id_pae).filter(Pathitems.id_pathid.in_(idis)).all()
#
#        return l1s
...
#        results = db.query(Moduleitem).filter(Moduleitem.id_pae == 1539105).all()
#        print "try"
#        for ra in results:
#            print ra.name
#        
...
#        smExps = []
#        for m in mods:
##            results = db.query(Moduleitem).filter(Moduleitem.id_pae == m).filter(Moduleitem.id_moe == Modelement.id).all()
#            results = db.query(Modelement).filter(Moduleitem.id_pae == m).filter(Moduleitem.id_moe == Modelement.id).all()
#            results2 = []
#            for r in results:
#                r.id_pae = m
#                print str(m) + " -_ " + str(r.name)
#                results2.append(r)
#                
#            smExps.extend(results2)
            
#        query = db.query(Moduleitem).filter(Moduleitem.id_pae.in_(mods))
#        print query
        print "------------- "
#        smExps = query.all()
    
#        print "INSIDE QUERY: " + str(smExps)
        return smExps             
